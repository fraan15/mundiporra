...
export function MatchCard({ match, onSaved, verticalScorePicker=false }) {
  ...
  const loadScorers=()=>{const codes=[match.team1_team?.fifa_code,match.team2_team?.fifa_code].filter(Boolean);if(scorerEnabled&&codes.length===2)return api(`/players?team_fifa_codes=${codes.join(",")}`).then(setPlayers).catch(()=>[]);return Promise.resolve([])};
  useEffect(()=>{loadScorers()},[match.id,scorerEnabled,match.team1_team?.fifa_code,match.team2_team?.fifa_code]);
  ...
  return <article className={`match-card ${match.status} ${match.in_play?"in-play":""} ${match.is_star?"star-match-card":""}`}>
    ...
    {match.betting_open && !user.is_read_only ? <div className="prediction">
      ...
      {verticalScorePicker
        ? <div className="detail-score-picker horizontal match-score-picker-horizontal"><HorizontalScoreControl team={match.team1} value={g1} onChange={setG1} onAdjust={delta=>adjust(setG1,g1,delta)}/><b>:</b><HorizontalScoreControl team={match.team2} value={g2} onChange={setG2} onAdjust={delta=>adjust(setG2,g2,delta)}/></div>
