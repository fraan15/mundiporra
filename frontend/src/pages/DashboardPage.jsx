      ...
      {visibleDays.length ? visibleDays.map(day => <article className="calendar-day" key={day.key}>
        ...
        <div>{day.matches.length ? (expanded ? day.matches : day.visibleMatches).map(match => <button type="button" className="calendar-match" key={match.id} onPointerDown={startMatchPointer} onPointerMove={moveMatchPointer} onPointerCancel={()=>{pointerRef.current=null}} onClick={event=>clickMatch(event, match)} onKeyDown={event=>openMatchOnKey(event, match)} aria-label={`Ver detalle de ${match.team1} contra ${match.team2}`}>
          ...
          <span className="calendar-match-meta">
            ...
            <span className={`calendar-close ${match.betting_open ? "open" : "closed"}`}><Clock3 size={12}/>{closeText(match, currentTime)}</span>
            {Boolean(match.is_star)&&<span className="calendar-star"><Star size={12} fill="currentColor"/> Partido Estrella x2</span>}
          </span>
