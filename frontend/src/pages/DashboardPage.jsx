...
function DashboardCalendar({ matches, expanded, onExpand, onCollapse, onOpenMatch, restoreScrollTop, user, currentTime }) {
  ...
  return <section className={`dashboard-calendar ${expanded ? "expanded" : ""}`} onClick={!expanded ? onExpand : undefined} onKeyDown={!expanded ? handleKeyDown : undefined} role={!expanded ? "button" : undefined} tabIndex={!expanded ? 0 : undefined} aria-label="Ampliar calendario de partidos">
    ...
    <div className="calendar-days" ref={daysRef}>
      {visibleDays.length ? visibleDays.map(day => <article className="calendar-day" key={day.key}>
        <h3>{day.title}</h3>
        <div>{day.matches.length ? (expanded ? day.matches : day.visibleMatches).map(match => <button type="button" className="calendar-match" key={match.id} onPointerDown={startMatchPointer} onPointerMove={moveMatchPointer} onPointerCancel={()=>{pointerRef.current=null}} onClick={event=>clickMatch(event, match)} onKeyDown={event=>openMatchOnKey(event, match)} aria-label={`Ver detalle de ${match.team1} contra ${match.team2}`}>
...
export function DashboardPage() {
  ...
  const {user}=useAuth(),navigate=useNavigate(),location=useLocation(),[data,setData]=useState(null),[activity,setActivity]=useState([]),[calendarMatches,setCalendarMatches]=useState([]),[calendarExpanded,setCalendarExpanded]=useState(()=>calendarReturnInfo?Boolean(calendarReturnInfo.expanded):true),[tick,setTick]=useState(Date.now()),[matchIndex,setMatchIndex]=useState(0),[liveMatchIndex,setLiveMatchIndex]=useState(0);
  ...
  <DashboardCalendar matches={calendarMatches} expanded={calendarExpanded} onExpand={()=>setCalendarExpanded(true)} onCollapse={(event)=>{event.stopPropagation();setCalendarExpanded(false)}} onOpenMatch={openCalendarMatch} restoreScrollTop={calendarRestoreScrollTop} user={user} currentTime={tick}/>
