...
export function HistoryPage() {
  ...
  return <div className="page"><section className="page-heading"><span className="eyebrow"><CalendarDays size={14}/> ARCHIVO DEL TORNEO</span><h1>Histórico diario</h1><p>Revive cada jornada, tus apuestas y los puntos repartidos.</p></section>
    <div className="day-grid">{days.map(day=><button className="day-card" key={day.match_date} onClick={()=>navigate(`/historico/${day.match_date}`)}><div className="calendar-tile"><strong>{new Date(`${day.match_date}T12:00:00`).getDate()}</strong><span>{new Date(`${day.match_date}T12:00:00`).toLocaleDateString("es-ES",{month:"short"}).toUpperCase()}</span></div><div><strong>{new Date(`${day.match_date}T12:00:00`).toLocaleDateString("es-ES",{weekday:"long",year:"numeric"})}</strong><span>{day.matches_count} partidos · {day.my_points} puntos ganados</span></div><ChevronRight/></button>)}</div>
  </div>;
}
...
export function DayHistoryPage() {
  ...
  return <div className="page"><button className="back-btn" onClick={()=>navigate("/historico")}><ArrowLeft size={16}/>Volver al histórico</button><section className="page-heading compact"><span className="eyebrow">RESUMEN DIARIO</span><h1>{new Date(`${date}T12:00:00`).toLocaleDateString("es-ES",{weekday:"long",day:"numeric",month:"long"})}</h1></section>
    <div className="history-layout"><div className="history-matches">{data.matches.map(m=><article className="history-match" key={m.id}><div><span>{m.match_time} · {m.stadium}</span><strong>{m.team1} <b>{m.status==="finished"?`${m.result_team1} – ${m.result_team2}`:"vs"}</b> {m.team2}</strong><small>{m.predicted_winner?`Tu apuesta: ${m.predicted_team1_goals} – ${m.predicted_team2_goals}`:"Sin apuesta"}</small></div><em>{m.total_points||0} pts</em>{data.predictions[m.id]&&<div className="day-reveals">{data.predictions[m.id].map((p,i)=><span key={i}><b>{p.username}</b> {p.predicted_team1_goals}–{p.predicted_team2_goals} · {p.total_points} pts</span>)}</div>}</article>)}</div>
