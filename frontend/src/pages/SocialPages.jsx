...
function PointsMatchRow({match,open,onToggle}){
 ...
 return <article className="points-match-row">
  <button type="button" className="points-match-toggle" aria-expanded={open} onClick={onToggle}><div className="points-match-summary"><time>{new Date(`${match.match_date}T12:00:00`).toLocaleDateString("es-ES",{weekday:"short",day:"2-digit",month:"short",year:"numeric"})}{match.match_time?` · ${match.match_time.slice(0,5)}`:""}</time><strong><span><Flag team={match.team1}/>{match.team1}</span><b>{match.result}</b><span>{match.team2}<Flag team={match.team2}/></span></strong><footer><span className={match.status==="finished"?"finished":"live"}>{match.status==="finished"?"Finalizado":"En vivo"}</span><small>Pronóstico {match.prediction}</small><em>{match.total_points>0?"+":""}{match.total_points} puntos</em></footer></div><ChevronDown className={open?"open":""}/></button>
