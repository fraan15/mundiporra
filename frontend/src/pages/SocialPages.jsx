...
export function PublicProfilePage(){
  const {id}=useParams(),navigate=useNavigate(),[data,setData]=useState(null),[historyPage,setHistoryPage]=useState(1),[pointsOpen,setPointsOpen]=useState(false),[openMatchId,setOpenMatchId]=useState(null),[avatarOpen,setAvatarOpen]=useState(false);
  const pageSize=5;
...
function PointsMatchRow({match,open,onToggle}){
 ...
 return <article className="points-match-row">
  ...
  {open&&<div className="points-match-body">
   <div className="points-match-ledger"><span>Puntos antes<strong>{match.points_before}</strong></span><span>Este partido<strong>{match.total_points>0?"+":""}{match.total_points}</strong><button type="button" onClick={()=>navigate(`/match/${match.match_id}`)}>Ver detalle</button></span><span>Puntos después<strong>{match.points_after}</strong></span></div>
