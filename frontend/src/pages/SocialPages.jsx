...
const consumePointsReturn=()=>{const returning=sessionStorage.getItem("returnToPointsDetail")==="1";if(returning)sessionStorage.removeItem("returnToPointsDetail");return returning};

export function ProfilePage(){
  const {user:authUser,setUser}=useAuth(); const [data,setData]=useState(null),[phrase,setPhrase]=useState(""),[displayName,setDisplayName]=useState(""),[saved,setSaved]=useState(false),[saveError,setSaveError]=useState(""),[avatarMessage,setAvatarMessage]=useState(""),[uploading,setUploading]=useState(false),[cropFile,setCropFile]=useState(null),[pointsOpen,setPointsOpen]=useState(consumePointsReturn);
  ...
  return <div className="page"><section className="profile-hero"><div className="profile-avatar-editor"><Avatar user={data.user} className="profile-avatar"/>{!authUser.is_read_only&&<><label className="avatar-upload"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={changeAvatar} disabled={uploading}/>{uploading?"Procesando...":data.user.avatar_url?"Cambiar foto":"Añadir foto"}</label>{data.user.avatar_url&&<button type="button" onClick={removeAvatar} disabled={uploading}>Eliminar</button>}</>}</div><div><span className="eyebrow">PERFIL DE JUGADOR</span><h1>{data.user.display_name||data.user.username}</h1><p>{authUser.is_read_only?"Solo lectura":data.user.role==="admin"?"Administrador":"Participante"} · Desde {new Date(data.user.created_at).toLocaleDateString("es-ES")}</p>{!authUser.is_read_only&&<small className="avatar-requirements">JPEG, PNG o WebP · máximo 5 MB · mínimo 100 × 100 px</small>}{avatarMessage&&<small className={avatarMessage.includes("actualizada")||avatarMessage.includes("eliminada")?"success-text":"error-text"}>{avatarMessage}</small>}</div></section>
    {pointsOpen&&<PointsDetailOverlay detail={data.points_detail} username={data.user.display_name||data.user.username} onClose={()=>setPointsOpen(false)}/>}<StatCards s={s} onPointsInfo={()=>setPointsOpen(true)}/>{!authUser.is_read_only&&<section className="content-card"><h2>Editar perfil</h2><div className="phrase-editor"><label>Nombre visible<input minLength="2" maxLength="40" value={displayName} onChange={e=>setDisplayName(e.target.value)} placeholder="Nombre visible"/></label><label>Frase visible<input maxLength="120" value={phrase} onChange={e=>setPhrase(e.target.value)} placeholder="Este año gano yo."/></label><button className="primary" onClick={save}><Edit3 size={16}/>Guardar</button></div><small>Puedes cambiar el nombre visible hasta 3 veces cada 24 horas. Tu usuario de acceso no cambia.</small>{saved&&<small className="success-text">Perfil actualizado.</small>}{saveError&&<small className="error-text">{saveError}</small>}</section>}
    {cropFile&&<AvatarCropper file={cropFile} onCancel={()=>setCropFile(null)} onConfirm={uploadAvatar}/>}<StatsSections stats={s} history={data.history}/><section className="content-card medals-card"><h2>Medallas <small>{s.badges.length}</small></h2><Badges badges={s.badges}/></section>
  </div>
}
...
export function PublicProfilePage(){
  const {id}=useParams(),navigate=useNavigate(),[data,setData]=useState(null),[historyPage,setHistoryPage]=useState(1),[pointsOpen,setPointsOpen]=useState(consumePointsReturn),[openMatchId,setOpenMatchId]=useState(null),[avatarOpen,setAvatarOpen]=useState(false);
  const pageSize=5;
  ...
  const matchDetails=new Map((data.points_detail?.matches||[]).map(match=>[Number(match.id),match]));
  return <div className="page">{avatarOpen&&<AvatarPreview user={data.user} onClose={()=>setAvatarOpen(false)}/>} {pointsOpen&&<PointsDetailOverlay detail={data.points_detail} username={data.user.display_name||data.user.username} onClose={()=>setPointsOpen(false)}/>}<button className="back-btn" onClick={()=>navigate(-1)}><ArrowLeft size={16}/>Volver</button><section className="profile-hero public">{data.user.avatar_url?<button type="button" className="profile-avatar-preview-trigger" onClick={()=>setAvatarOpen(true)} aria-label={`Ampliar foto de perfil de ${data.user.display_name||data.user.username}`}><Avatar user={data.user} className="profile-avatar"/></button>:<Avatar user={data.user} className="profile-avatar"/>}<div><span className="eyebrow">FICHA DEPORTIVA</span><h1>{data.user.display_name||data.user.username}</h1><small className="profile-real-username">@{data.user.username}</small><blockquote>“{data.user.personal_phrase||"Todavía sin frase personal."}”</blockquote></div><b>#{s.position}</b></section><StatCards s={s} onPointsInfo={()=>setPointsOpen(true)}/><StatsSections stats={s} history={data.history}/><section className="content-card medals-card"><h2>Medallas <small>{s.badges.length}</small></h2><Badges badges={s.badges}/></section><section className="content-card"><h2>Historial visible</h2><p className="prediction-history-hint">Toca un partido para desplegar toda la información de puntuación.</p><div className="prediction-history">{visiblePredictions.map(p=>{const detail=matchDetails.get(Number(p.id));return <PredictionHistoryRow key={p.id} prediction={p} detail={detail} open={openMatchId===p.id} onToggle={()=>detail&&setOpenMatchId(openMatchId===p.id?null:p.id)}/>})}</div>{totalHistoryPages>1&&<nav className="pagination" aria-label="Paginación del historial visible"><button disabled={historyPage===1} onClick={()=>{setHistoryPage(historyPage-1);setOpenMatchId(null)}}><ChevronLeft/>Anterior</button><span>Página {historyPage} de {totalHistoryPages}</span><button disabled={historyPage===totalHistoryPages} onClick={()=>{setHistoryPage(historyPage+1);setOpenMatchId(null)}}>Siguiente<ChevronRight/></button></nav>}</section></div>
}
function AvatarPreview({user,onClose}){
 ...
 return <div className="avatar-preview-overlay" role="dialog" aria-modal="true" aria-label={`Foto de perfil de ${name}`} onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><div className="avatar-preview-dialog"><button type="button" className="avatar-preview-close" onClick={onClose} aria-label="Cerrar foto ampliada"><X/></button><img src={user.avatar_url} alt={`Foto de perfil de ${name}`}/></div></div>
}
function PredictionHistoryRow({prediction,detail,open,onToggle}){
 const match=detail,p=prediction;
 if(!match)return <article className="prediction-history-card unavailable"><div className="prediction-history-summary"><time>{p.match_date}</time><strong><Flag team={p.team1}/>{p.team1} <b>{p.predicted_team1_goals}–{p.predicted_team2_goals}</b> {p.team2}<Flag team={p.team2}/></strong><em>{Number(p.total_points)>0?"+":""}{p.total_points} pts</em></div></article>;
 const earnedRules=match.rules.filter(rule=>rule.points>0),missedRules=match.rules.filter(rule=>rule.points===0);
 const finished=p.status==="finished";
 return <article className={`prediction-history-card ${open?"open":""}`}><button type="button" className="prediction-history-summary" aria-expanded={open} onClick={onToggle}><time>{new Date(`${match.match_date}T12:00:00`).toLocaleDateString("es-ES",{weekday:"short",day:"2-digit",month:"short"})}{match.match_time?` · ${match.match_time.slice(0,5)}`:""}</time><strong><span><Flag team={match.team1}/>{match.team1}</span><b>{match.result}</b><span>{match.team2}<Flag team={match.team2}/></span></strong><footer><em>{match.total_points>0?"+":""}{match.total_points} puntos</em><i className={finished?"finished":"live"}>{finished?"Finalizado":"En vivo"}</i></footer><ChevronDown className={open?"open":""}/></button>{open&&<div className="prediction-history-detail"><p className="prediction-history-pick">Pronóstico <strong>{match.prediction}</strong><span>Goleador <strong>{match.predicted_scorer_name||"Sin goleador"}</strong></span></p><div className="points-match-ledger"><span>Puntos antes<strong>{match.points_before}</strong></span><span>Este partido<strong>{match.total_points>0?"+":""}{match.total_points}</strong></span><span>Puntos después<strong>{match.points_after}</strong></span></div><div className="points-rule-grid">{earnedRules.map(rule=><div key={rule.label} className="earned"><Check size={15}/><strong>{rule.label}</strong><span>{match.multiplier>1?`${rule.base_points} x ${match.multiplier} = ${rule.points}`:`${rule.points} pts`}</span><small>{rule.text}</small></div>)}</div>{earnedRules.length===0&&<p className="empty-state">No sumó puntos en este partido.</p>}{match.is_star&&<p className="star-explanation"><Star size={15} fill="currentColor"/> Partido Estrella: aciertos x{match.multiplier}.</p>}<p className="points-formula">Suma: {match.formula} puntos · Total: {match.points_after} puntos.</p>{missedRules.length>0&&<div className="missed-rules"><strong>Aciertos no conseguidos</strong>{missedRules.map(rule=><span key={rule.label}><b>{rule.label}</b>{rule.text}</span>)}</div>}</div>}</article>
}
function PointsDetailOverlay({detail,username,onClose}){
 const savedUi=useMemo(()=>{try{return JSON.parse(sessionStorage.getItem("pointsDetailUi")||"null")}catch{return null}},[]),[matchesOpen,setMatchesOpen]=useState(savedUi?.matchesOpen??true),[openMatchId,setOpenMatchId]=useState(savedUi?.openMatchId??null),[matchesPage,setMatchesPage]=useState(savedUi?.matchesPage??1);
 useEffect(()=>{sessionStorage.setItem("pointsDetailUi",JSON.stringify({matchesOpen,openMatchId,matchesPage}))},[matchesOpen,openMatchId,matchesPage]);
 useEffect(()=>{const close=event=>{if(event.key==="Escape")onClose()};document.addEventListener("keydown",close);return()=>document.removeEventListener("keydown",close)},[onClose]);
 const matches=detail?.matches||[],sortedMatches=[...matches].sort((a,b)=>{
  const dateCompare=new Date(`${b.match_date}T${b.match_time||"00:00:00"}`)-new Date(`${a.match_date}T${a.match_time||"00:00:00"}`);
  return dateCompare||Number(b.id)-Number(a.id);
 }),matchesPageSize=10,totalMatchesPages=Math.max(1,Math.ceil(sortedMatches.length/matchesPageSize)),visibleMatches=sortedMatches.slice((matchesPage-1)*matchesPageSize,matchesPage*matchesPageSize);
 ...
 return <div className="team-detail-overlay points-detail-overlay" role="dialog" aria-modal="true" aria-label="Detalle de puntos" onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}>
  <section className="team-detail-panel points-detail-panel">
   ...
   <div className="points-ledger-summary">
    ...
    <article><span>Puntos en partidos</span><strong>{detail?.automatic_points||0}</strong><small>Ganador {detail?.winner_points||0} · Exacto {detail?.exact_result_points||0} · Goleador {detail?.scorer_points||0}</small></article>
    <article><span>Ajustes</span><strong>{signed(detail?.adjustment_points)}</strong></article>
    <article><span>Partidos revisados</span><strong>{detail?.finished_matches||0}</strong><small>{detail?.matches_with_points||0} con puntos · {detail?.matches_without_points||0} sin puntos</small></article>
   </div>
   <section className="points-detail-section points-collapsible-section"><button type="button" className="points-section-toggle" aria-expanded={matchesOpen} onClick={()=>setMatchesOpen(!matchesOpen)}><span><h2>Todos los partidos</h2><small>{sortedMatches.length} partido{sortedMatches.length===1?"":"s"} revisado{sortedMatches.length===1?"":"s"} · del más reciente al más antiguo</small></span><ChevronDown className={matchesOpen?"open":""}/></button>{matchesOpen&&(sortedMatches.length?<><div className="points-match-list">{visibleMatches.map(match=><PointsMatchRow key={match.id} match={match} open={openMatchId===match.id} onToggle={()=>setOpenMatchId(openMatchId===match.id?null:match.id)}/>)}</div>{totalMatchesPages>1&&<nav className="pagination" aria-label="Paginación de todos los partidos revisados"><button disabled={matchesPage===1} onClick={()=>{setMatchesPage(page=>page-1);setOpenMatchId(null)}}><ChevronLeft/>Anterior</button><span>Página {matchesPage} de {totalMatchesPages}</span><button disabled={matchesPage===totalMatchesPages} onClick={()=>{setMatchesPage(page=>page+1);setOpenMatchId(null)}}>Siguiente<ChevronRight/></button></nav>}</>:<p className="empty-state">Todavía no hay partidos revisados.</p>)}</section>
...
function PointsMatchRow({match,open,onToggle}){
 ...
 return <article className="points-match-row">
  <button type="button" className="points-match-toggle" aria-expanded={open} onClick={onToggle}><div className="points-match-summary"><header><time>{new Date(`${match.match_date}T12:00:00`).toLocaleDateString("es-ES",{weekday:"short",day:"2-digit",month:"short",year:"numeric"})}{match.match_time?` · ${match.match_time.slice(0,5)}`:""}</time><i className={match.status==="finished"?"finished":"live"}>{match.status==="finished"?"Finalizado":"En vivo"}</i></header><strong><span><Flag team={match.team1}/><em>{match.team1}</em></span><b>{match.result}</b><span><em>{match.team2}</em><Flag team={match.team2}/></span></strong><footer><small>Pronóstico {match.prediction}</small><em>{match.total_points>0?"+":""}{match.total_points} puntos</em></footer></div><ChevronDown className={open?"open":""}/></button>
  {open&&<div className="points-match-body">
   <div className="points-match-ledger"><span>Puntos antes<strong>{match.points_before}</strong></span><span>Este partido<strong>{match.total_points>0?"+":""}{match.total_points}</strong><button type="button" onClick={()=>{sessionStorage.setItem("returnToPointsDetail","1");navigate(`/match/${match.match_id}`)}} aria-label="Ver detalle del partido" title="Ver detalle del partido"><Info size={15}/></button></span><span>Puntos después<strong>{match.points_after}</strong></span></div>
   ...
   <p className="points-formula">Suma del partido: {match.formula} puntos · Total después: {match.points_after} puntos.</p>
   {missedRules.length>0&&<div className="missed-rules"><strong>Aciertos no conseguidos</strong>{missedRules.map(rule=><span key={rule.label}><b>{rule.label}</b>{rule.text}</span>)}</div>}
  </div>}
 </article>
}
 ...
 return <article className={open?"activity-open":""}>
  ...
  <div>
   ...
   {open&&breakdown&&<div className="activity-breakdown">
    ...
    {breakdown.rules.map(rule=><span key={rule.label}><b>{rule.label}{rule.detail&&<small title={rule.detail}>({rule.detail})</small>}</b><em>{breakdown.multiplier>1?`${rule.base_points} de ${rule.description} x ${breakdown.multiplier} = ${rule.earned_points}`:`${rule.base_points} de ${rule.description} = ${rule.earned_points}`}</em></span>)}
...
function MatchPredictionSummary({match,user}){
 ...
 const total=Number(match.total_points)||0,multiplier=Number(match.scoring_multiplier)||1;
 const rules=[
  ["Ganador","Has acertado el ganador o el empate.",Number(match.winner_points)||0],
  ["Resultado exacto","Has acertado el marcador exacto.",Number(match.exact_result_points)||0],
  ["Goleador",match.predicted_scorer?.name?`Has acertado el goleador: ${match.predicted_scorer.name}.`:"Has acertado el goleador.",Number(match.scorer_points)||0]
 ].filter(([, , points])=>points>0);
 const formula=rules.map(([, , points])=>points).join(" + ");
 if(user.is_read_only)return <div className="locked-prediction-summary"><strong className="big-score prediction-score-card">Sin participación</strong><p>Puedes ver el partido, participantes, distribución y comentarios sin intervenir.</p></div>;
 return <div className="locked-prediction-summary">
  <strong className="big-score prediction-score-card">{match.prediction_id?`${match.predicted_team1_goals} – ${match.predicted_team2_goals}`:"Sin pronóstico"}</strong>
  {match.predicted_scorer&&<p className="prediction-scorer-pill">Goleador elegido: <b>{match.predicted_scorer.name}</b></p>}
  {match.status==="finished"?<><button type="button" className="prediction-points-pill" aria-expanded={open} onClick={()=>setOpen(!open)}><Info size={15}/>{total>0?"+":""}{total} puntos obtenidos<ChevronDown className={open?"open":""}/></button>{open&&<div className="prediction-points-breakdown"><strong>Desglose de puntos</strong>{rules.length?rules.map(([label,text])=><span key={label}><b>{label}</b><small>{text}</small></span>):<p>No has sumado puntos en este partido.</p>}{Number(match.is_star)===1&&<p><Star size={14} fill="currentColor"/> Partido Estrella: los aciertos se multiplican x{multiplier}.</p>}{rules.length>0&&<p>Suma final: {rules.length>1?`${formula} = `:""}{total} puntos.</p>}</div>}</>:<p>Las apuestas de este partido están cerradas.</p>}
 </div>;
}
...
export function MatchDetailPage(){
 ...
 useEffect(()=>{load()},[id]);
	 ...
	 const loadPickScorers=(onlyIfEmpty=false)=>{if(onlyIfEmpty&&players.length>0)return Promise.resolve(players);const currentMatch=data?.match,codes=[currentMatch?.team1_team?.fifa_code,currentMatch?.team2_team?.fifa_code].filter(Boolean);if(scorerEnabled&&codes.length===2)return api(`/players?team_fifa_codes=${codes.join(",")}`).then(setPlayers).catch(()=>[]);return Promise.resolve([])};
	 useEffect(()=>{loadPickScorers()},[data?.match.id,scorerEnabled]);
