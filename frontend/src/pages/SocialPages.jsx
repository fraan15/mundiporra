...
export function PublicProfilePage(){
  const {id}=useParams(),navigate=useNavigate(),[data,setData]=useState(null),[historyPage,setHistoryPage]=useState(1),[pointsOpen,setPointsOpen]=useState(false),[selectedMatch,setSelectedMatch]=useState(null),[avatarOpen,setAvatarOpen]=useState(false);
  const pageSize=5;
...
function MatchPointsBanner({match,username,onClose}){
 useEffect(()=>{const close=event=>{if(event.key==="Escape")onClose()};document.addEventListener("keydown",close);return()=>document.removeEventListener("keydown",close)},[onClose]);
 const earnedRules=match.rules.filter(rule=>rule.points>0),missedRules=match.rules.filter(rule=>rule.points===0);
 return <div className="team-detail-overlay match-points-overlay" role="dialog" aria-modal="true" aria-label={`Puntuación de ${match.team1} contra ${match.team2}`} onMouseDown={event=>{if(event.target===event.currentTarget)onClose()}}><section className="team-detail-panel match-points-banner"><button className="team-detail-close" aria-label="Cerrar puntuación" onClick={onClose}><X/></button><header><span className="eyebrow">PUNTUACIÓN DEL PARTIDO</span><h1><Flag team={match.team1}/>{match.team1} <b>{match.result}</b> {match.team2}<Flag team={match.team2}/></h1><p>{username} pronosticó <strong>{match.prediction}</strong> · Goleador: <strong>{match.predicted_scorer_name||"Sin goleador"}</strong></p></header><div className="points-match-ledger"><span>Puntos antes<strong>{match.points_before}</strong></span><span>Este partido<strong>{match.total_points>0?"+":""}{match.total_points}</strong></span><span>Puntos después<strong>{match.points_after}</strong></span></div><section><h2>Desglose de puntos</h2><div className="points-rule-grid">{earnedRules.map(rule=><div key={rule.label} className="earned"><Check size={15}/><strong>{rule.label}</strong><span>{match.multiplier>1?`${rule.base_points} base x ${match.multiplier} = ${rule.points}`:`${rule.points} pts`}</span><small>{rule.text}</small></div>)}</div>{earnedRules.length===0&&<p className="empty-state">No sumó puntos en este partido.</p>}</section>{match.is_star&&<p className="star-explanation"><Star size={15} fill="currentColor"/> Partido Estrella: los aciertos se multiplican x{match.multiplier}.</p>}<p className="points-formula">Suma del partido: {match.formula} puntos · Total después: {match.points_after} puntos.</p>{missedRules.length>0&&<div className="missed-rules"><strong>Aciertos no conseguidos</strong>{missedRules.map(rule=><span key={rule.label}><b>{rule.label}</b>{rule.text}</span>)}</div>}</section></div>
}
...
function PointsMatchRow({match,open,onToggle}){
 ...
 return <article className="points-match-row">
  ...
  {open&&<div className="points-match-body">
   <div className="points-match-ledger"><span>Puntos antes<strong>{match.points_before}</strong></span><span>Este partido<strong>{match.total_points>0?"+":""}{match.total_points}</strong></span><span>Puntos después<strong>{match.points_after}</strong></span></div>
