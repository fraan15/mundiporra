...
function PointsMatchRow({match,open,onToggle}){
 ...
 return <article className="points-match-row">
  <button type="button" className="points-match-toggle" aria-expanded={open} onClick={onToggle}><div><strong><Flag team={match.team1}/>{match.team1} {match.result} {match.team2}<Flag team={match.team2}/></strong><span>{new Date(`${match.match_date}T12:00:00`).toLocaleDateString("es-ES",{day:"2-digit",month:"short",year:"numeric"})}{match.match_time?` · ${match.match_time.slice(0,5)}`:""} · Pronóstico {match.prediction} · {match.predicted_winner_label}</span></div><b>{match.total_points>0?"+":""}{match.total_points}</b><ChevronDown className={open?"open":""}/></button>
