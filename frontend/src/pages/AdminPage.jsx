...
function AdminPredictionReview({ match, onClose, onCorrected }) {
  ...
  return <section className="admin-form prediction-review">
    ...
    {!data ? <p>Cargando apuestas…</p> : data.predictions.length === 0 ? <div className="admin-list-empty">No hay apuestas registradas.</div> : <div className="prediction-admin-list">{data.predictions.map((row) => <article key={row.id} className={editing?.id === row.id ? "editing" : ""}>
      ...
      {editing?.id === row.id && <div className="prediction-correction-editor"><div className="correction-score"><label>{match.team1}<input type="number" min="0" value={editing.g1} onChange={(e) => setEditing({...editing, g1:e.target.value})}/></label><b>–</b><label>{match.team2}<input type="number" min="0" value={editing.g2} onChange={(e) => setEditing({...editing, g2:e.target.value})}/></label></div><label>Ganador derivado<input readOnly value={winner(editing.g1, editing.g2)}/></label>{Boolean(Number(match.scorer_enabled)) && <label>Goleador pronosticado{Number(editing.g1) + Number(editing.g2) === 0 ? <input readOnly value="Sin goleador"/> : <SearchSelect items={availablePlayers} value={editing.scorer_id} onChange={(player) => setEditing({...editing, scorer_id:player?.id || null})} placeholder="Selecciona goleador..." renderItem={(player) => <><strong>{player.name}</strong><small>{player.team_name} · {player.position}</small></>}/>}</label>}<label className="correction-reason">Motivo<textarea minLength={5} maxLength={500} rows={2} placeholder="Explica por qué se corrige…" value={editing.reason} onChange={(e) => setEditing({...editing, reason:e.target.value})}/></label><p className="correction-warning">Se guardarán los valores anteriores, los nuevos, el administrador y el motivo.</p><div><button type="button" className="primary" disabled={editing.reason.trim().length < 5 || editing.g1 === "" || editing.g2 === "" || (scorerRequired && !editing.scorer_id)} onClick={save}>Confirmar corrección</button><button type="button" className="secondary" onClick={() => setEditing(null)}>Cancelar</button></div></div>}
    </article>)}</div>}
  </section>;
}
