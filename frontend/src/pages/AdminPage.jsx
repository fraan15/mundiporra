...
function AdminPredictionReview({ match, onClose, onCorrected }) {
  ...
  const begin = () => {
    ...
    setEditing({
      id: row.id || null, user_id: row.user_id, creating: !row.id, username: row.username,
      g1: String(row.predicted_team1_goals ?? 0), g2: String(row.predicted_team2_goals ?? 0),
      scorer_id: row.id
        ? (row.predicted_scorer_id || (row.predicted_team1_goals + row.predicted_team2_goals === 0 ? NO_SCORER_ID : null))
        : (Number(match.scorer_enabled) ? NO_SCORER_ID : null),
      reason: "",
    });
  };
  ...
  const save = async () => {
    setError("");
    try {
      const result = await api(editing.creating ? `/admin/matches/${match.id}/predictions` : `/admin/matches/${match.id}/predictions/${editing.id}`, { method: editing.creating ? "POST" : "PATCH", body: {
        user_id: editing.user_id, predicted_team1_goals: Number(editing.g1), predicted_team2_goals: Number(editing.g2), predicted_scorer_id: editing.scorer_id, reason: editing.reason,
      }});
  ...
  const isEditing = (row) => editing && (editing.creating ? editing.user_id === row.user_id : editing.id === row.id);
  const passwordGate = (row) => passwordRequest?.row === row && <form className="prediction-password-gate" onSubmit={(event) => { event.preventDefault(); begin(); }}><label>Contraseña de super administrador<input type="password" autoFocus value={passwordRequest.password} onChange={(event) => setPasswordRequest({...passwordRequest, password:event.target.value})}/></label>{passwordRequest.error && <span>{passwordRequest.error}</span>}<div><button type="submit" className="primary">Desbloquear</button><button type="button" className="secondary" onClick={() => setPasswordRequest(null)}>Cancelar</button></div></form>;
  const predictionEditor = (row) => isEditing(row) && <div className="prediction-correction-editor"><div className="correction-score detail-score-picker horizontal"><HorizontalScoreControl team={match.team1} value={editing.g1} onChange={(value) => setEditing({...editing, g1:value})} onAdjust={(delta) => adjustScore("g1", delta)}/><b>:</b><HorizontalScoreControl team={match.team2} value={editing.g2} onChange={(value) => setEditing({...editing, g2:value})} onAdjust={(delta) => adjustScore("g2", delta)}/></div><div className="correction-derived"><span>Ganador</span><strong>{winner(editing.g1, editing.g2)}</strong></div>{Boolean(Number(match.scorer_enabled)) && <div className="correction-scorer"><span>Goleador pronosticado</span>{Number(editing.g1) + Number(editing.g2) === 0 ? <div className="scorer-selected-banner readonly"><div><span>Goleador elegido</span><strong>Sin goleador</strong><small>Marcador 0-0</small></div></div> : <ScorerPicker players={availablePlayers} value={editing.scorer_id} onChange={(scorerId) => setEditing({...editing, scorer_id:scorerId})} matchLabel={`${match.team1} - ${match.team2}`}/>}</div>}<label className="correction-reason">Motivo<textarea minLength={5} maxLength={500} rows={2} placeholder={editing.creating ? "Ej.: apuesta comunicada por teléfono…" : "Explica por qué se corrige…"} value={editing.reason} onChange={(e) => setEditing({...editing, reason:e.target.value})}/></label><p className="correction-warning">{editing.creating ? "Se insertará fuera de plazo y la operación quedará auditada." : "Se guardarán los valores anteriores, los nuevos, el administrador y el motivo."}</p><div className="correction-actions"><button type="button" className="primary" disabled={editing.reason.trim().length < 5 || editing.g1 === "" || editing.g2 === "" || (scorerRequired && !editing.scorer_id)} onClick={save}>{editing.creating ? "Añadir apuesta" : "Confirmar corrección"}</button><button type="button" className="secondary" onClick={() => setEditing(null)}>Cancelar</button></div></div>;
  return <section className="admin-form prediction-review">
    <header><div><span className="eyebrow">REVISIÓN SEGURA</span><h3>Apuestas: {match.team1} – {match.team2}</h3><p>El partido permanece {match.status === "finished" ? "finalizado" : match.status === "open" ? "en curso" : "cerrado"}. Cada corrección queda auditada{match.status === "finished" ? " y recalcula los puntos" : ""}.</p></div><button type="button" className="secondary" onClick={onClose}>Cerrar</button></header>
    {error && <div className="alert error">{error}</div>}
    {!data ? <p>Cargando apuestas…</p> : <div className="prediction-review-columns">
      <div><h4>Apuestas registradas <span>{data.predictions.length}</span></h4>{data.predictions.length === 0 ? <div className="admin-list-empty">No hay apuestas registradas.</div> : <div className="prediction-admin-list">{data.predictions.map((row) => <article key={row.id} className={isEditing(row) ? "editing" : ""}><div className="prediction-admin-summary"><strong>{row.username}</strong><b>Oculta</b><span>Marcador oculto</span><span>Goleador oculto</span>{match.status === "finished" && <small>Puntos ocultos</small>}<button type="button" onClick={() => requestEdit(row)}>Corregir</button></div>{passwordGate(row)}{predictionEditor(row)}</article>)}</div>}</div>
      <div><h4>Usuarios sin apostar <span>{data.missing_users?.length || 0}</span></h4>{!data.missing_users?.length ? <div className="admin-list-empty">Todos los usuarios han apostado.</div> : <div className="prediction-admin-list missing">{data.missing_users.map((row) => <article key={row.user_id} className={isEditing(row) ? "editing" : ""}><div className="prediction-missing-summary"><strong>{row.username}</strong>{match.status === "finished" ? <span>Resultado finalizado</span> : <button type="button" onClick={() => requestEdit(row)}>Añadir apuesta</button>}</div>{passwordGate(row)}{predictionEditor(row)}</article>)}</div>}</div>
    </div>}
  </section>;
}
