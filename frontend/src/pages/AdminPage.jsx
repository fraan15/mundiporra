...
import { Activity, Calculator, CalendarSearch, Check, Download, MessageSquareText, Plus, Settings, Shield, Trash2, Users } from "lucide-react";
...
function AdminSettings(){
 const [form,setForm]=useState(null),[notice,setNotice]=useState(""),[syncing,setSyncing]=useState(false),[syncError,setSyncError]=useState(""),[jsonStatus,setJsonStatus]=useState(null);useEffect(()=>{Promise.all([api("/admin/settings"),api("/admin/worldcup-json-status")]).then(([settings,status])=>{setForm(settings);setJsonStatus(status)})},[]);if(!form)return null;
 ...
 const syncJson=async()=>{setSyncing(true);setSyncError("");setNotice("");try{const result=await api("/admin/sync-worldcup-json",{method:"POST"});setJsonStatus(result);setNotice(`JSON sincronizado: ${result.matches} partidos · ${new Date(result.synced_at).toLocaleString("es-ES")}.`)}catch(error){setSyncError(error.message)}finally{setSyncing(false)}};
 return <section className="admin-section"><Notice text={notice}/>{syncError&&<div className="alert error">{syncError}</div>}<form className="admin-form" onSubmit={save}><h3>Reglas generales</h3><div className="form-grid"><label>Nombre de la porra<input value={form.pool_name} onChange={e=>setForm({...form,pool_name:e.target.value})}/></label><label>Puntos por ganador<input type="number" min="0" step="1" value={form.winner_points} onChange={e=>setForm({...form,winner_points:e.target.value})}/></label><label>Puntos por exacto<input type="number" min="0" step="1" value={form.exact_result_points} onChange={e=>setForm({...form,exact_result_points:e.target.value})}/></label><label>Puntos por goleador<input type="number" min="0" step="1" value={form.scorer_points} onChange={e=>setForm({...form,scorer_points:e.target.value})}/></label><label>Minutos antes del partido<input type="number" min="0" step="1" value={form.auto_close_minutes_before} onChange={e=>setForm({...form,auto_close_minutes_before:e.target.value})}/></label><label className="toggle"><input type="checkbox" checked={form.auto_close_enabled==="1"} onChange={e=>setForm({...form,auto_close_enabled:e.target.checked?"1":"0"})}/>Activar cierre automático</label></div><button className="primary">Guardar configuración</button></form><div className="action-panel"><Download size={32}/><h3>Información de equipos y partidos</h3><p>Descarga de nuevo el JSON del Mundial y actualiza resultados, goleadores y estadísticas mostrados en las fichas.</p><button type="button" className="primary" disabled={syncing} onClick={syncJson}>{syncing?"Sincronizando…":"Sincronizar info JSON"}</button><div className="json-sync-status"><span>Última descarga</span><strong>{jsonStatus?.synced_at?new Date(jsonStatus.synced_at).toLocaleString("es-ES"):"Todavía no disponible"}</strong>{jsonStatus?.synced_at&&<small>{jsonStatus.matches} partidos recibidos</small>}</div></div></section>;
}
function AdminLogs(){
