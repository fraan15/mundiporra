...
export function MatchesPage() {
  ...
  return <div className="page matches-page-redesign">
    ...
    {loading?<div className="matches-agenda-skeleton"><i/><i/><i/></div>:grouped.length?<div className="matches-agenda">{grouped.map(([date,items])=><section className="matches-day" key={date}>
      ...
      <div>{items.map(match=>{const liveScore=liveScores[match.id];const liveNow=liveScore?.available&&liveScore.state==="in"&&!liveScore.completed;return <button type="button" className="agenda-match-row" key={match.id} onClick={()=>openMatch(match.id)}>
        <span className="agenda-time"><strong>{liveNow?(liveScore.clock||"EN JUEGO"):hasResult(match)?"FIN":localMatchTime(match,user.country_code)}</strong><small className={liveNow?"live":hasResult(match)?"":"upcoming"}>{liveNow?"MINUTO":hasResult(match)?"FINAL":"PRÓXIMAMENTE"}</small></span>
        <span className="agenda-fixture"><span><strong>{match.team1}</strong><Flag team={match.team1} teamData={match.team1_team}/></span><b>{liveScore?.available?<EspnLiveScore data={liveScore}/>:hasResult(match)?`${match.result_team1} — ${match.result_team2}`:"VS"}</b><span><Flag team={match.team2} teamData={match.team2_team}/><strong>{match.team2}</strong></span></span>
