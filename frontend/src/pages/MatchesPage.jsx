  ...
  return <div className="page matches-page-redesign">
    ...
    {loading?<div className="matches-agenda-skeleton"><i/><i/><i/></div>:grouped.length?<div className="matches-agenda">{grouped.map(([date,items])=><section className="matches-day" key={date}>
      ...
      <div>{items.map(match=>{const liveScore=liveScores[match.id];const liveNow=liveScore?.available&&liveScore.state==="in"&&!liveScore.completed;return <button type="button" className="agenda-match-row" key={match.id} onClick={()=>openMatch(match.id)}>
        <span className="agenda-time">{liveNow?<EspnLiveScore data={liveScore}/>:<><strong>{hasResult(match)?"FIN":localMatchTime(match,user.country_code)}</strong><small className={hasResult(match)?"":"upcoming"}>{hasResult(match)?"FINAL":"PRÓXIMAMENTE"}</small></>}</span>
