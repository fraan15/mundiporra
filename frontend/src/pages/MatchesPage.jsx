...
export function MatchesPage() {
  ...
  return <div className="page matches-page-redesign">
    ...
    {loading?<div className="matches-agenda-skeleton"><i/><i/><i/></div>:grouped.length?<div className="matches-agenda">{grouped.map(([date,items])=><section className="matches-day" key={date}>
      ...
      <div>{items.map(match=>{const liveScore=liveScores[match.id];const showEspn=liveScore?.available&&liveScore.score&&!hasResult(match);const espnFinal=showEspn&&(liveScore.completed||liveScore.espn_completed);return <button type="button" className={`agenda-match-row ${match.is_star?"agenda-star-match":""}`} key={match.id} onClick={()=>openMatch(match.id)}>
        <span className="agenda-time">{showEspn?espnFinal?<><strong className="espn-final">FIN</strong><small className="espn-final-source">ESPN</small></>:<small className="live">En vivo</small>:<><strong>{hasResult(match)?"FIN":localMatchTime(match,user.country_code)}</strong><small className={hasResult(match)?"":"upcoming"}>{hasResult(match)?"FINAL":"PRÓXIMAMENTE"}</small></>}</span>
