...
export function LeaderboardPage() {
  ...
  return <div className="page"><section className="page-heading"><span className="eyebrow"><Trophy size={14}/> CLASIFICACIÓN GENERAL</span><h1>La carrera por la copa</h1><p>Ranking global, diferencia con el líder y perfiles de cada participante.</p></section>
    ...
    <div className="table-card"><table><thead><tr><th>Pos.</th><th>Participante</th><th>Total</th><th>Ganador</th><th>Exacto</th><th>Ajustes</th><th>Pronósticos</th><th>Aciertos</th></tr></thead><tbody>{rows.map((row,index)=><tr key={row.id}><td><b><Position index={index} total={rows.length}/></b></td><td className="clickable-user" onClick={()=>navigate(`/usuario/${row.id}`)}><span className="mini-avatar">{row.username[0].toUpperCase()}</span><strong>{row.username}</strong>{row.personal_phrase&&<small>{row.personal_phrase}</small>}</td><td><strong className="points">{row.total_points}</strong>{index>0&&<small className="leader-gap">-{rows[0].total_points-row.total_points}</small>}</td><td>{row.winner_points}</td><td>{row.exact_result_points}</td><td className={row.adjustments<0?"negative":""}>{row.adjustments>0?"+":""}{row.adjustments}</td><td>{row.predicted_matches}</td><td><span className="hit"><Target size={14}/>{row.winner_hits} / {row.exact_hits}</span></td></tr>)}</tbody></table></div>
  </div>;
}

function Position({index,total}) {
  const icon=index===0?"👑":index===total-1?"😳":"";
  return <>#{index+1}{icon&&<span role="img" aria-label={index===0?"Primer clasificado":"Último clasificado"}> {icon}</span>}</>;
}
