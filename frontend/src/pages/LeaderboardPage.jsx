...
export function LeaderboardPage() {
  ...
  return <div className="page"><section className="page-heading"><span className="eyebrow"><Trophy size={14}/> CLASIFICACIÓN GENERAL</span><h1>La carrera por la copa</h1><p>Ranking global y perfiles de cada participante.</p></section>
    ...
    <div className="table-card"><table><thead><tr><th>Pos.</th><th>Participante</th><th>Total</th><th>Ganador</th><th>Exacto</th><th>Goleador</th><th>Ajustes</th><th>Pronósticos</th><th>Aciertos G/E/GL</th></tr></thead><tbody>{rows.map((row,index)=><tr key={row.id}><td><b><Position index={index} total={rows.length}/></b></td><td className="clickable-user" onClick={()=>navigate(`/usuario/${row.id}`)}><Avatar user={row} className="mini-avatar"/><strong>{row.username}</strong>{row.personal_phrase&&<small>{row.personal_phrase}</small>}</td><td><strong className="points">{row.total_points}</strong></td><td>{row.winner_points}</td><td>{row.exact_result_points}</td><td>{row.scorer_points}</td><td className={row.adjustments<0?"negative":""}>{row.adjustments>0?"+":""}{row.adjustments}</td><td>{row.predicted_matches}</td><td><span className="hit"><Target size={14}/>{row.winner_hits} / {row.exact_hits} / {row.scorer_hits}</span></td></tr>)}</tbody></table></div>
  </div>;
}
...
function exportRows(rows,type){const separator=type==="csv"?";":"\t";const text=[["Posición","Usuario","Puntos","Aciertos ganador","Exactos","Goleadores"],...rows.map((r,i)=>[i+1,r.username,r.total_points,r.winner_hits,r.exact_hits,r.scorer_hits])].map(r=>r.join(separator)).join("\n");const blob=new Blob([`\uFEFF${text}`],{type:type==="csv"?"text/csv":"application/vnd.ms-excel"});const link=document.createElement("a");link.href=URL.createObjectURL(blob);link.download=`clasificacion.${type==="csv"?"csv":"xls"}`;link.click();URL.revokeObjectURL(link.href)}
