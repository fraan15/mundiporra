...
export function ChatPage() {
  ...
  const streamRef = useRef(null), initialLoad = useRef(true), fileRef = useRef(null), touch = useRef(null);
  ...
  const load = async () => { const data = await api("/chat"); setMessages(data); if (initialLoad.current) { initialLoad.current = false; if (!user.is_read_only) await api("/chat/read", { method: "POST" }); scrollToLatest("auto", true); } };
  ...
  const submit = async (event) => { event.preventDefault(); if ((!text.trim() && !media) || sending) return; setSending(true); try { await api("/chat", { method: "POST", body: { message: text, reply_to_id: reply?.id || null, media } }); setText(""); setReply(null); setMedia(null); setGifOpen(false); await load(); scrollToLatest("smooth"); } finally { setSending(false); } };
